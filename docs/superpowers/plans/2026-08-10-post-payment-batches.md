...
- **R2 — never hand-write `src/data/site-content.seed.json`.** `scripts/sync-seed.mjs` regenerates the seed FROM `data/site-content.json`, so a hand-edited seed is destroyed by the next `npm run sync-seed`. Every content-data change edits `data/site-content.json`, then runs `npm run sync-seed`, then commits **both** files together, and ends with `npm run sync-seed -- --check` printing `✓ seed is in sync with data/site-content.json`.
