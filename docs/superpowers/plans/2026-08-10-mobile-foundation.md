...
> **✅ SEED DRIFT RESOLVED 2026-08-10 (commit `53ce8a5`).** The owner reviewed the drift and chose to keep their `/admin` edits: Jubilee Hills now leads the studios array (array order and `displayOrder` finally agree), the About headline drops its trailing period, and `stories[].authorId` materialised as `''`. `npm run sync-seed -- --check` passes. Nothing to do here — proceed.
>
> **Still true and load-bearing:** `data/site-content.json` is **gitignored** (`.gitignore:8-9`). It can never appear in a commit — a `git add data/site-content.json` step silently no-ops, and any clean-tree gate assuming it landed is misleading. `src/data/site-content.seed.json` is the tracked artifact and the fallback the app serves on a fresh clone or in CI. Where R2 below says "commits **both** files in the same commit", read it as: edit the live document, run `npm run sync-seed`, and commit **the seed**.
...
- **R6 — Test-count arithmetic.** This plan runs **second**, so its baseline is Plan 1's end state: **29 files / 317 tests, all green**. (The pre-Plan-1 repo is 26 files / 279 tests, verified on Node v24.18.0 / vitest 4.1.10 — that is *not* this plan's starting point.) This plan's own delta is **+5 files, +35 tests**: `image-variants.test.ts` (8), `image-downscale.test.ts` (7), `image-dimensions.test.ts` (8), `public-urls.test.ts` (6), `social-url.test.ts` (6) — 8+7+8+6+6 = 35. So 29 + 5 = ~~34 files~~ and 317 + 35 = ~~352 tests~~. **Plan 1 DID land on a different number — re-base from the delta.** Its actual end state is **31 files / 351 tests** (the owner's own CSP commit `c5c2ea8` added `csp-headers.test.ts` with 6 tests, and Plan 1's final-review fix wave added `client-bundle.test.ts` with 28). So this plan's real target is **31 + 5 = 36 files** and **351 + 35 = 386 tests**. Verify the actual starting count with `npx vitest run` at Task 1 and carry the delta from whatever you observe.
...
- **Zero new hardcoded user-visible strings.** Every `aria-label` this plan writes reads through `label(content.labels, …)`. Every `alt` reads `content.hero.posterAlt`.
- **CORRECTED 2026-08-10 after Plan 1 shipped — the hero alt has a blank fallback, preserve it.** Plan 1's final-review fix wave added `src/lib/hero-defaults.ts` (a zero-import module exporting `HERO_POSTER_ALT_DEFAULT`), and `Hero.tsx` now renders `alt={content.hero.posterAlt.trim() !== '' ? content.hero.posterAlt : HERO_POSTER_ALT_DEFAULT}`. That fallback exists because clearing the field in `/admin/hero` would otherwise emit `alt=""`, silently converting a content-bearing image into a decorative one. **Task 6 rewrites this exact block into a `<picture>` — it must carry the fallback across unchanged.** An earlier draft of this plan asserted "there is no module-level `POSTER_ALT` constant"; that is no longer true and must not be used to justify deleting the fallback.
