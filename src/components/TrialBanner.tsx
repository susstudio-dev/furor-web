...
export function TrialBanner({ content }: { content: SiteContent }) {
  ...
  return (
    <section className="container-x py-6 sm:py-8">
      <div className="relative overflow-hidden rounded-3xl border border-ember-500/30 bg-gradient-to-r from-ember-500/10 via-gold-500/8 to-ember-500/10 px-5 py-5 sm:px-8 sm:py-6">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-8">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              {t.eyebrow ? (
                <span className="pill bg-ember-500/20 text-ember-300">{t.eyebrow}</span>
              ) : null}
              {t.when ? (
                <span className="text-xs uppercase tracking-widest text-cream/55">{t.when}</span>
              ) : null}
            </div>
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            {useLinkPrimary ? (
              <>
                <Link
                  href={t.ctaHref}
                  className="inline-flex items-center gap-2 rounded-full bg-ember-500 text-cream px-5 py-2.5 text-sm font-semibold hover:bg-ember-600 transition"
                >
                  {t.ctaLabel || 'See more'}
                </Link>
                {t.ctaContext ? (
