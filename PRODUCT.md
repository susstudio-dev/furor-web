...
- Performance budget (binding): LCP < 2.5 s, CLS < 0.1, INP < 200 ms on Lighthouse mobile; first-load JS **total < 115 KB gzip per route**, of which **app-authored client JS < 12 KB gzip per route** (the React/Next framework floor alone measures 100.14 KB gz, so a single sub-100 KB number was unreachable without leaving Next.js; home (`/`) measured **138.12 KB total / 37.98 KB app-authored** as of 2026-08-24, on this branch — vs. **136.74 / 36.60** measured the same way on `main`, so this branch added ~1.4 KB, not the reduction Task 11 predicted; the 117.51/17.37 figure this bullet cited before 2026-08-13 was stale even against the `main` it was meant to describe. These figures drift with every change, so treat `npm run build && npm run audit:bundle` as the source of truth, not this parenthetical — note `npm run build` currently hangs on an interactive eslint install prompt unless eslint is installed; run `npx next build --no-lint` instead); LCP image **< 45 KB AVIF at DPR ≤ 2** (measured 36,741 B for the 750w variant) and **≤ 60 KB at DPR 3** (measured 54,341 B for the 1125w variant that `sizes="100vw"` selects on the sharpest phones — the owner accepted this in Task 6 over a visible quality drop from q40 re-encoding or an upscaled DPR-1 image; `npm run audit:images` negotiates the real `<picture>` srcset per DPR and reports both figures directly — cite it for this check); hero video < 2 MB. **8 public routes breach the JS budget as of 2026-08-24** — `/`, `/dance-styles`, `/about`, `/dance-styles/[slug]`, `/instructors`, `/batches`, `/contact` and `/faqs` are all over the 115/12 KB limits (`/` is worst at 138.12/37.98; `/stories`, `/stories/[slug]` and `/welcome/[track]` stay well under budget) — re-run `npm run audit:bundle` for current numbers.
